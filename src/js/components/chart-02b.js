...
// ===== chartFour
var chart04 = () => { //Change chartname here
  ...
  // 
  const chartSelector = document.querySelectorAll("#chartFour"); //Change chart ID here

  if (chartSelector.length) {
    const chartFour = new ApexCharts( //change chartnumber here
      document.querySelector("#chartFour"), 
      chartFourOptions //change options
    );
    chartFour.render(); //change chart name
    
    const getDataButton = document.getElementById("getDataButton04"); //Change button ID here
    getDataButton.addEventListener('click', async function (e) {
      const sensor = "Volume"; //Change sensor
      try{
        const start_date = queryDates["WSN2b1"][0]; //Change date ID
        const end_date = queryDates["WSN2b2"][0]; //Change date ID
        console.log(start_date);
        console.log(end_date);
        if (end_date > start_date){
          const [Xvals, Yvals] = await readFromDatabase(sensor, start_date, end_date);
          chartFour.updateSeries([{ //Change chart
            name: "Volume", //Change type
            data: Yvals
          }]);
          chartFour.updateOptions({ //Change Chart
            xaxis: {
              type: "category",
              categories: Xvals,
              labels: {
