...
const citiesRef = collection(db, "WSN3"); //Change WSN here
...
// ===== chartSeven
var chart07 = () => { //Change chartname here
  ...
  // 
  const chartSelector = document.querySelectorAll("#chartSeven"); //Change chart ID here

  if (chartSelector.length) {
    const chartSeven = new ApexCharts( //change chartnumber here
      document.querySelector("#chartSeven"), 
      chartSevenOptions //change options
    );
    chartSeven.render(); //change chart name
    
    const getDataButton = document.getElementById("getDataButton07"); //Change button ID here
    getDataButton.addEventListener('click', async function (e) {
      const sensor = "Volume"; //Change sensor
      try{
        const start_date = queryDates["WSN3b1"][0]; //Change date ID
        const end_date = queryDates["WSN3b2"][0]; //Change date ID
        console.log(start_date);
        console.log(end_date);
        if (end_date > start_date){
          const [Xvals, Yvals] = await readFromDatabase(sensor, start_date, end_date);
          chartSeven.updateSeries([{ //Change chart
            name: "Volume", //Change type
            data: Yvals
          }]);
          chartSeven.updateOptions({ //Change Chart
            xaxis: {
              type: "category",
              categories: Xvals,
              labels: {
