...
// ===== chartFourteen
var chart14 = () => { //Change chartname here
  ...
  // 
  const chartSelector = document.querySelectorAll("#chartFourteen"); //Change chart ID here

  if (chartSelector.length) {
    const chartFourteen = new ApexCharts( //change chartnumber here
      document.querySelector("#chartFourteen"), 
      chartFourteenOptions //change options
    );
    chartFourteen.render(); //change chart name
    
    const getDataButton = document.getElementById("getDataButton14"); //Change button ID here
    getDataButton.addEventListener('click', async function (e) {
      const sensor = "Rate"; //Change sensor
      try{
        const start_date = queryDates["WSN5c1"][0]; //Change date ID
        const end_date = queryDates["WSN5c2"][0]; //Change date ID
        console.log(start_date);
        console.log(end_date);
        if (end_date > start_date){
          const [Xvals, Yvals] = await readFromDatabase(sensor, start_date, end_date);
          chartFourteen.updateSeries([{ //Change chart
            name: "Rate", //Change type
            data: Yvals
          }]);
          chartFourteen.updateOptions({ //Change Chart
            xaxis: {
              type: "category",
              categories: Xvals,
              labels: {
