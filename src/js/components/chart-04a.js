...
// ===== chartNine
var chart09 = () => { //Change chartname here
  ...
  // 
  const chartSelector = document.querySelectorAll("#chartNine"); //Change chart ID here

  if (chartSelector.length) {
    const chartNine = new ApexCharts( //change chartnumber here
      document.querySelector("#chartNine"),
      chartNineOptions //change options
    );
    chartNine.render(); //change chart name
    
    const getDataButton = document.getElementById("getDataButton09"); //Change button ID here
    getDataButton.addEventListener('click', async function (e) {
      const sensor = "Pressure"; //Change sensor
      try{
        const start_date = queryDates["WSN4a1"][0]; //Change date ID
        const end_date = queryDates["WSN4a2"][0]; //Change date ID
        console.log(start_date);
        console.log(end_date);
        if (end_date > start_date){
          const [Xvals, Yvals] = await readFromDatabase(sensor, start_date, end_date);
          chartNine.updateSeries([{ //Change chart
            name: "Pressure", //Change type
            data: Yvals
          }]);
          chartNine.updateOptions({ //Change Chart
            xaxis: {
              type: "category",
              categories: Xvals,
              labels: {
