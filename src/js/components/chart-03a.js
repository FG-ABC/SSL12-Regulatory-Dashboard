...
// ===== chartSix
var chart06 = () => { //Change chartname here
  ...
  // 
  const chartSelector = document.querySelectorAll("#chartSix"); //Change chart ID here

  if (chartSelector.length) {
    const chartSix = new ApexCharts( //change chartnumber here
      document.querySelector("#chartSix"),
      chartSixOptions //change options
    );
    chartSix.render(); //change chart name
    
    const getDataButton = document.getElementById("getDataButton06"); //Change button ID here
    getDataButton.addEventListener('click', async function (e) {
      const sensor = "Pressure"; //Change sensor
      try{
        const start_date = queryDates["WSN3a1"][0]; //Change date ID
        const end_date = queryDates["WSN3a2"][0]; //Change date ID
        console.log(start_date);
        console.log(end_date);
        if (end_date > start_date){
          const [Xvals, Yvals] = await readFromDatabase(sensor, start_date, end_date);
          chartSix.updateSeries([{ //Change chart
            name: "Pressure", //Change type
            data: Yvals
          }]);
          chartSix.updateOptions({ //Change Chart
            xaxis: {
              type: "category",
              categories: Xvals,
              labels: {
