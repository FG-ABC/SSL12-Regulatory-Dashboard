...
// ===== chartEight
var chart08 = () => { //Change chartname here
  ...
  // 
  const chartSelector = document.querySelectorAll("#chartEight"); //Change chart ID here

  if (chartSelector.length) {
    const chartEight = new ApexCharts( //change chartnumber here
      document.querySelector("#chartEight"), 
      chartEightOptions //change options
    );
    chartEight.render(); //change chart name
    
    const getDataButton = document.getElementById("getDataButton08"); //Change button ID here
    getDataButton.addEventListener('click', async function (e) {
      const sensor = "Rate"; //Change sensor
      try{
        const start_date = queryDates["WSN3c1"][0]; //Change date ID
        const end_date = queryDates["WSN3c2"][0]; //Change date ID
        console.log(start_date);
        console.log(end_date);
        if (end_date > start_date){
          const [Xvals, Yvals] = await readFromDatabase(sensor, start_date, end_date);
          chartEight.updateSeries([{ //Change chart
            name: "Rate", //Change type
            data: Yvals
          }]);
          chartEight.updateOptions({ //Change Chart
            xaxis: {
              type: "category",
              categories: Xvals,
              labels: {
