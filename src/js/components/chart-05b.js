...
// ===== chartThirteen
var chart13 = () => { //Change chartname here
  ...
  // 
  const chartSelector = document.querySelectorAll("#chartThirteen"); //Change chart ID here

  if (chartSelector.length) {
    const chartThirteen = new ApexCharts( //change chartnumber here
      document.querySelector("#chartThirteen"), 
      chartThirteenOptions //change options
    );
    chartThirteen.render(); //change chart name
    
    const getDataButton = document.getElementById("getDataButton13"); //Change button ID here
    getDataButton.addEventListener('click', async function (e) {
      const sensor = "Volume"; //Change sensor
      try{
        const start_date = queryDates["WSN5b1"][0]; //Change date ID
        const end_date = queryDates["WSN5b2"][0]; //Change date ID
        console.log(start_date);
        console.log(end_date);
        if (end_date > start_date){
          const [Xvals, Yvals] = await readFromDatabase(sensor, start_date, end_date);
          chartThirteen.updateSeries([{ //Change chart
            name: "Volume", //Change type
            data: Yvals
          }]);
          chartThirteen.updateOptions({ //Change Chart
            xaxis: {
              type: "category",
              categories: Xvals,
              labels: {
