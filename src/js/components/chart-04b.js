...
// ===== chartTen
var chart10 = () => { //Change chartname here
  ...
  // 
  const chartSelector = document.querySelectorAll("#chartTen"); //Change chart ID here

  if (chartSelector.length) {
    const chartTen = new ApexCharts( //change chartnumber here
      document.querySelector("#chartTen"), 
      chartTenOptions //change options
    );
    chartTen.render(); //change chart name
    
    const getDataButton = document.getElementById("getDataButton10"); //Change button ID here
    getDataButton.addEventListener('click', async function (e) {
      const sensor = "Volume"; //Change sensor
      try{
        const start_date = queryDates["WSN4b1"][0]; //Change date ID
        const end_date = queryDates["WSN4b2"][0]; //Change date ID
        console.log(start_date);
        console.log(end_date);
        if (end_date > start_date){
          const [Xvals, Yvals] = await readFromDatabase(sensor, start_date, end_date);
          chartTen.updateSeries([{ //Change chart
            name: "Volume", //Change type
            data: Yvals
          }]);
          chartTen.updateOptions({ //Change Chart
            xaxis: {
              type: "category",
              categories: Xvals,
              labels: {
